...
class db_session():
	...
	def get_uid_by_number(self, phone_number):
	...
	"""
	def register_user(self, username, password_enc, phone_numbers):
		db = self.mongoclient.txti
	...
	def get_loginid_from_number(self, phone_number, api_name):
		db = self.mongoclient.txti
	...
	def get_loginid_from_uid(self, _id, api_name):
		db = self.mongoclient.txti
	...
	def get_login_from_number(self, phone_number, api_name):
		return self.mongoclient.txti["api_"+api_name].find_one({"_id":
			self.get_loginid_from_number(phone_number, api_name)
			})

	def get_login_from_uid(self, _id, api_name):
		return self.mongoclient.txti["api_"+api_name].find_one({"_id":
			self.get_loginid_from_uid(_id, api_name)
			})
...
if __name__ == "__main__":
	...
	try:
		...
		uid = session.get_uid_by_number("2153611306")
	...
	print session.get_login_from_number("2153611306", "fakeapi")
	print session.get_login_from_uid(uid, "fakeapi")
