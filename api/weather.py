...
def weather_current(country, city):
    ...
    windchill = str(parsed_json['current_observation']['windchill_c' if metric else 'windchill_f']) + ' C' if metric else ' F'
    feelslike = str(parsed_json['current_observation']['feelslike_c' if metric else 'feelslike_f']) + ' C' if metric else ' F'

    if not 'NA' in windchill: return "It is currently %s and %s (windchill: %s) in %s." % (condition.lower(), temp, windchill, location)
    else:  return "It is currently %s and %s (feels like %s) in %s." % (condition.lower(), temp, feelslike, location)
